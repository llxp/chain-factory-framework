...
class TaskQueueHandlers():
    ...
    async def wait_for_task(
        ...
        """
        credentials = await self._credentials_pool.get_credentials(namespace)
        ...
        try:
            ...
            self._task_waiter[namespace] = TaskWaiter(
                self.task_queue_clients.mongodb_client(
                    mongodb_credentials, namespace)
            )
            await self._task_waiter[namespace].wait_for_task_name(
                task_name, arguments
            )
